...
- A standalone program `mainCode` that generates a PNG file (ex: `gradient.png`)
...
### 2) A new executable target: `mainCode`
...
- `add_executable(mainCode mainCode.cpp)`
...
- Define an executable target `mainCode` that:
...
target_link_libraries(mainCode PRIVATE cs4212-util)
target_link_libraries(mainCode PRIVATE ${Boost_PROGRAM_OPTIONS_LIBRARIES})
target_link_libraries(mainCode PRIVATE PNG::PNG)
target_link_libraries(mainCode PRIVATE ZLIB::ZLIB)
```
...
./src/mainCode
```
from your buildVCPkg directory (Windows: `.\src\Debug\mainCode.exe`, see below)
...
Same commands up to `cmake --build .`, but CMake builds a `Debug` configuration
by default and drops the executable inside a `Debug` folder:
...
(See [head-scratchers](head-scratchers.md#windows) for why the path differs.)
...
- Are you running `mainCode` from the correct folder?
